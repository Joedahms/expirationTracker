...
float calibration_factor = 3300.00; //400150.00 worked for the small setup I created. Will need to be redone for the final "scale" created
...
void loop() {
  ...
  for (int i = 0; i < MOV_AVG_SAMPLES-1; i++)     // Shifting samples the moving average is looking at
  ...
  if (count == 3) {              //will check if weight is on scale every 20 iterations
    if (abs(exponentialSumming(movingAverage, 0)) >= abs(exponentialSumming(movingAverage, 0)) - (prevSample * epsilon) ) { //weight on scale was removed
  ...
  Serial.print(" kg (moving exp average)"); //Change this to kg and re-adjust the calibration factor if you follow SI units like a sane person
...
float exponentialSumming (float data, bool reset) { //implementing a moving exponential average filter
